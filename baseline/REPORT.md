...
**Key finding — platform effect is target-set-dependent:** Using all targets, SecAct shows significant bulk–pseudobulk differences in most strata (11/13 GTEx tissues, 5/11 TCGA cancers), while CytoSig shows almost none (1/13, 0/11). However, when restricted to the same 32 shared targets (Matched tabs), both methods show no significant platform differences (CytoSig: 0/13, 0/11; SecAct: 0/13, 1/11). The apparent platform sensitivity is driven entirely by SecAct's ~1,000 additional secreted-protein targets — many with weaker expression–activity signals that are more susceptible to technical differences between bulk and pseudobulk. Core cytokine targets are platform-robust. Full per-stratum results in [statistical methods](stats_section_4.1.html#cross-platform-comparison).
